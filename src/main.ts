...
export class Main extends Base {
  ...
  async run() {
    ...
    let start = [-76, 42];
    let zoom = 6;
    if ((<any>params)["load"] === "rwvm") {
      this.loadGpxFromUrl("./20240510.gpx");
      this.loadGpxFromUrl("./20240715.gpx");
      this.loadGpxFromUrl("./20240716.gpx");
      this.loadGpxFromUrl("./20240717.gpx");
      this.loadGpxFromUrl("./20240719.gpx");
      start = [-3.7864640, -38.5528030];
      zoom = 17;
    }
  ...
  rebuildActivityTable() {
    ...
    let datasets = [];
    for (let i = 0; i < this.player.activities.length; i++) {
      let iniTime: number = this.player.activities[0].startDateTime?.getTime() || 0
      const activity = this.player.activities[i];
      datasets.push({ borderColor: 'rgb(' + this.colors![i].join() + ')', data: activity.points.map((item) => ({ x: iniTime++, y: item[3] })) })
    }

    const canvas: any = (<HTMLCanvasElement>this.getById('chart')).getContext('2d');
    if (this.chart)
      this.chart.destroy()
    ...
    for (let i = 0; i < this.player.activities.length; i++) {
      ...
      if (activity) {
        ...
        if (tr) {
          ...
          tr.children[6].innerHTML = activity.hr?.toString() || "";
        }
      }
    }
  }
